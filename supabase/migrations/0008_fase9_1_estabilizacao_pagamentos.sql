-- Fase 9.1: estabiliza RPCs instaladas; preserva tabelas, RLS e constraints.
drop function if exists public.create_subscription_payment(uuid,text,text,text,text);
create function public.create_subscription_payment(p_plan_id uuid,p_billing_cycle text,p_payment_method text,p_transaction_id text default null,p_notes text default null)
...
begin
 select business_id into v_business from public.business_users where user_id=auth.uid() limit 1; if v_business is null then raise exception 'Negócio não encontrado'; end if;
 select * into v_plan from public.plans where id=p_plan_id and active; if v_plan.id is null then raise exception 'Plano indisponível'; end if;
 ...
 -- A assinatura ativa nunca é rebaixada a pending: o pagamento proposto guarda o novo plan_id.
 select * into v_sub from public.subscriptions where business_id=v_business and status in ('active','pending','past_due','suspended') order by created_at desc limit 1 for update;
 ...
 elsif v_sub.status='pending' then update public.subscriptions set plan_id=v_plan.id,billing_cycle=p_billing_cycle,auto_renew=false where id=v_sub.id returning * into v_sub;
 end if;
 v_reference:='CC-'||to_char(now(),'YYYYMMDD')||'-'||upper(substr(replace(gen_random_uuid()::text,'-',''),1,6));
 insert into public.subscription_payments(business_id,subscription_id,plan_id,amount,currency,billing_cycle,payment_method,reference,transaction_id,notes,paid_at,confirmed_at,confirmed_by) values(v_business,v_sub.id,v_plan.id,v_amount,v_plan.currency,p_billing_cycle,p_payment_method,v_reference,nullif(trim(p_transaction_id),''),p_notes,null,null,null) returning * into v_payment;
...
exception when unique_violation then
 if sqlerrm like '%transaction_id%' then raise exception 'Esta transação já foi utilizada'; end if; raise;
end $$;
drop function if exists public.confirm_subscription_payment(uuid,boolean,text);
create function public.confirm_subscription_payment(p_payment_id uuid,p_confirm boolean,p_notes text default null)
returns public.subscription_payments language plpgsql security definer set search_path=public as $$
declare v_payment public.subscription_payments; v_sub public.subscriptions; v_start timestamptz;
begin
 if not public.is_platform_admin() then raise exception 'Sem permissão de plataforma'; end if;
 select * into v_payment from public.subscription_payments where id=p_payment_id for update; if v_payment.id is null or v_payment.status<>'pending' then raise exception 'Pagamento não está pendente'; end if;
 ...
 insert into public.subscription_audit_logs(business_id,actor_id,action,details) values(v_payment.business_id,auth.uid(),'payment_confirmed',jsonb_build_object('payment_id',v_payment.id)); return v_payment;
end $$;
drop function if exists public.expire_due_subscriptions();
create function public.expire_due_subscriptions() returns integer language plpgsql security definer set search_path=public as $$ declare v_count integer; begin if not public.is_platform_admin() then raise exception 'Sem permissão de plataforma'; end if; with changed as (update public.subscriptions set status='expired' where status='active' and current_period_end<now() returning business_id,id) insert into public.subscription_audit_logs(business_id,actor_id,action,details) select business_id,auth.uid(),'subscription_expired',jsonb_build_object('subscription_id',id) from changed; get diagnostics v_count=row_count; return v_count; end $$;
