...
import { BUSINESS_TIMEZONE, zonedShortDate, zonedStartOfDay } from "@/lib/timezone";
...
export default async function DashboardPage({
  ...
  const [{ data: items }, { data: customers }] = await Promise.all([
    saleIds.length
      ...
      : Promise.resolve({ data: [] as { sale_id: string; product_id: string; quantity: number }[] }),
    customerIds.length
  ...
  // Stock, mais abaixo, continua a usar só `activeProducts`.
  const soldProductIds = [...new Set((items ?? []).map((i) => i.product_id))];
  const { data: soldProducts } = soldProductIds.length
    ? await supabase.from("products").select("id,name").in("id", soldProductIds)
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-3">
        <div>
          <h1 className="text-xl font-semibold text-ink">Olá, {profile?.name?.split(" ")[0] ?? ""} 👋</h1>
          <p className="text-sm text-ink/60">Como está o seu negócio hoje?</p>
        </div>
        <Link
        ...
        {(Object.entries(periods) as [Period, string][]).map(([value, label]) => (
          <Link
            ...
            className={`shrink-0 rounded-full px-3 py-2 text-sm ${
              value === period ? "bg-brand text-white" : "border border-line bg-white text-ink/60"
            }`}
          >
            {label}
          </Link>
      ...
      <section className="rounded-xl border border-line bg-white p-4">
        ...
        {chart.every((p) => p.value === 0) && sales?.length === 0 ? (
          <p className="py-10 text-center text-sm text-ink/50">Ainda não existem vendas neste período.</p>
        ) : (
          <svg viewBox="0 0 300 130" className="mt-4 h-36 w-full" role="img" aria-label="Gráfico de vendas">
            {chart.map((point, i) => {
              const x = chart.length === 1 ? 150 : 12 + (i * 276) / (chart.length - 1);
              const y = 112 - (point.value / chartMax) * 88;
              const prev = chart[i - 1];
              const prevX = chart.length === 1 ? 150 : 12 + ((i - 1) * 276) / (chart.length - 1);
              const prevY = prev ? 112 - (prev.value / chartMax) * 88 : y;
              return (
                <g key={point.key}>
                  <title>{`${point.label}: ${formatMT(point.value)}`}</title>
                  {i > 0 && <line x1={prevX} y1={prevY} x2={x} y2={y} stroke="#168f5b" strokeWidth="2" />}
                  <circle cx={x} cy={y} r="4" fill="#168f5b" />
                  <text x={x} y="128" textAnchor="middle" fontSize="9" fill="#697386">
                    {point.label}
                  </text>
                </g>
              );
      ...
      <section className="rounded-xl border border-line bg-white p-4">
        <div className="flex justify-between">
          <div>
            <h2 className="font-semibold text-ink">Por receber</h2>
            <p className="mt-1 text-lg font-semibold text-alert">{formatMT(outstanding)}</p>
            <p className="text-sm text-ink/60">
              {debtCustomers} cliente{debtCustomers === 1 ? "" : "s"} com dívida
            </p>
          </div>
          <Link href="/dashboard/dividas" className="text-sm font-medium text-brand">
            Ver dívidas →
          </Link>
        </div>
        {outstanding === 0 && <p className="mt-3 text-sm text-ink/60">Tudo certo! Não existem valores por receber.</p>}
      </section>

      <Section title="Produtos mais vendidos" link="/dashboard/vendas" action="Ver vendas">
        {topProducts.length ? (
          topProducts.map((p, i) => (
            <p key={p.name} className="flex justify-between py-2 text-sm">
              <span>
                {["🥇", "🥈", "🥉"][i]} {p.name}
              </span>
              <span className="text-ink/60">{p.quantity} unidades</span>
            </p>
          ))
        ) : (
      ...
      <Section title="Stock" link="/dashboard/produtos" action="Ver stock">
      ...
      <Section title="Últimas vendas" link="/dashboard/vendas" action="Ver todas →">
        {sales?.slice(0, 5).map((s) => (
          <div key={s.id} className="flex justify-between border-t border-line py-3 text-sm">
            <div>
              <p className="font-medium text-ink">
                {s.customer_id ? customerNames.get(s.customer_id) ?? "Cliente" : "Cliente avulso"}
              </p>
              <p className="text-ink/50">{zonedShortDate(new Date(s.created_at))}</p>
            </div>
            <b>{formatMT(s.total_amount)}</b>
          </div>
        ))}
        {!sales?.length && (
...
function Card({ label, value, green }: { label: string; value: number; green?: boolean }) {
  return (
    <div className="min-w-0 rounded-xl border border-line bg-white p-3">
      <p className="truncate text-xs text-ink/50">{label}</p>
      <p className={`mt-1 break-words text-sm font-semibold leading-snug sm:text-base ${green ? "text-brand" : "text-ink"}`}>
        {formatMT(value)}
      </p>
    </div>
  );
  ...
  return (
    <section className="rounded-xl border border-line bg-white p-4">
      <div className="mb-3 flex justify-between">
        ...
        <Link href={link} className="text-sm font-medium text-brand">
          {action}
        </Link>
      </div>
      {children}
