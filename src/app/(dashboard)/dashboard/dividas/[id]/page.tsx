...
export default async function DebtDetailPage({
  ...
  const { data: debt } = await supabase
    .from("debts")
    .select(
      "id, customer_id, sale_id, total_amount, amount_paid, status, created_at"
    )
    .eq("id", params.id)
  ...
  const [{ data: customer }, { data: sale }, { data: payments }] =
    await Promise.all([
      supabase
        .from("customers")
    ...
    <div className="space-y-6">
      <div>
        ...
        <h1 className="mt-2 text-xl font-semibold text-ink">
          Dívida de {customer?.name ?? "cliente"}
        </h1>

        <p className="mt-1 text-sm text-ink/60">
          Venda relacionada:{" "}
          {sale
            ? `${formatMT(sale.total_amount)} · ${new Date(
                sale.created_at
      ...
      <section className="grid grid-cols-2 gap-3">
        <div className="rounded-xl border border-line bg-white p-4">
          <p className="text-xs text-ink/50">Valor total</p>
          <p className="mt-1 font-semibold text-ink">
            {formatMT(debt.total_amount)}
          </p>
        </div>

        <div className="rounded-xl border border-line bg-white p-4">
          <p className="text-xs text-ink/50">Valor pago</p>
          <p className="mt-1 font-semibold text-ink">
            {formatMT(debt.amount_paid)}
          </p>
        </div>
        ...
        <div className="rounded-xl border border-line bg-white p-4">
          <p className="text-xs text-ink/50">Estado</p>
          <p className="mt-1 font-semibold text-ink">
      ...
      <section className="space-y-3">
        ...
        {payments?.length === 0 && (
          <p className="rounded-xl border border-dashed border-line p-4 text-sm text-ink/50">
            Esta dívida ainda não possui pagamentos.
          </p>
        )}
        ...
        <div className="space-y-2">
          {payments?.map((payment) => (
            <div
              key={payment.id}
              className="flex justify-between rounded-xl border border-line bg-white p-3"
            >
              <div>
