...
export default async function DividasPage({ searchParams }: { searchParams: { customer?: string } }) {
  ...
  return (
    <div className="space-y-5">
      <div><h1 className="text-xl font-semibold text-ink">Dívidas</h1><p className="mt-1 text-sm text-ink/60">Acompanhe os valores a receber.</p></div>
      <div className="space-y-2">
        {debts?.length === 0 && <p className="rounded-xl border border-dashed border-line p-6 text-center text-ink/50">Ainda não há dívidas registadas.</p>}
        {debts?.map((debt) => {
